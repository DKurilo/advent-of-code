...
springScriptToInput :: [String] -> [Int]
springScriptToInput instrs = (map ord . intercalate "\n") instrs <> [10]

parseOutput :: [Int] -> String
parseOutput [] = ""
parseOutput (score : ns) = (map chr . reverse) ns <> "\n" <> show score

startSpringBot :: Program -> [String] -> String
startSpringBot pr script = parseOutput . pOut . execute $ pr {pIn = springScriptToInput script}

part1Solution :: Program -> String
part1Solution pr = startSpringBot pr springScript
  where
    springScript =
      [ "NOT B J",
        "NOT C T",
...
part2Solution pr = startSpringBot pr springScript
  where
    springScript =
      [ "NOT B J",
        "NOT C T",
