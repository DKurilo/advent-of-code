...
instance Read Program where
  readPrec = do
    mem <- fmap fromList . lift . P.many1 $ do
      ...
      P.optional . P.char $ ','
      return n
    return $ Program mem [] [] 0 Running 0
...
part1Solution p = maximum . map (chainExec p 0) . permutations $ [0 .. 4]

part2Solution :: Program -> Int
part2Solution p =
  maximum
    . map (head . pOut . last . chainProgramExec 0 . map (\n -> execute $ p {pIn = [n], pId = n}))
    . permutations
    $ [5 .. 9]
